...
import com.jason.rxhttp.utils.IOUtils;
 ...
 * time   : 2019/07/10  14:15
 * desc   : OkHttp拦截器，主要用于打印日志
 * version: 1.0
 */
public class HttpLoggingInterceptor implements Interceptor {
    ...
    private void log(String message) {
        ...
        //  把4*1024的MAX字节打印长度改为3000字符数
        int max_str_length = 3000;
        //大于3000时
        if (message.length()>max_str_length) {
            while (message.length() > max_str_length) {
                logger.log(colorLevel, message.substring(0, max_str_length));
